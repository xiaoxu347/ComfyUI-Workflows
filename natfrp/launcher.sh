...
if command -v docker &>/dev/null && [[ $1 != "direct" ]]; then
    ...
    mkdir -p /kaggle/working/etc/natfrp || log_W "无法创建 /etc/natfrp 文件夹, 配置可能无法在容器重启后正常保存"
...
if ! id -u natfrp &>/dev/null; then
    ...
    useradd -r -m -s /sbin/nologin -d /kaggle/working natfrp
else
...
if [[ $do_download == 1 ]]; then
    ...
    curl -Lo - "https://nya.globalslb.net/natfrp/client/launcher-unix/latest/natfrp-service_linux_$arch.tar.zst" |
        tar -xI zstd -C /kaggle/working/ --overwrite
...
cat >"/kaggle/working/etc/systemd/system/natfrp.service" <<EOF
[Unit]
...
# Update config file
config_file=/kaggle/working/.config/natfrp-service/config.json
